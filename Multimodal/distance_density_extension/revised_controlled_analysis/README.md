...
distance–density analysis. It is **additive**: nothing in the parent package
[`../`](../) is modified, and the submission-era aggregate files, tables and
figures there remain unchanged.

Everything here is regenerated from the sanitized aggregate CSVs in
[data/](data/). Raw predictions, dataset files, model checkpoints and full
...
density gate do?". Families 3 and 4 answer the different question "at the same
amount removed, which rule loses fewer baseline detections?".
...
* **At an approximately matched point budget** (p90 gate, every setting matched to
  within 0.506%), the two rules need very different numbers of boxes to remove the
  same number of points, and the re-cut distance-only comparator keeps the lower
  lost-ratio at all five thresholds
...
Support density is retained in the method as an **object-level characterization of
camera–LiDAR support** and as a **candidate-restriction criterion** within a chosen
distance range. These files do not show that it ranks removal candidates better
than proximity on this holdout and this fixed pretrained model.

## Quick start

```bash
pip install -r ../requirements.txt     # numpy, pandas, matplotlib; only matplotlib is used here
...
Every script reads only the CSVs in [data/](data/) and writes only into
[outputs/](outputs/). `make_manifest.py` rewrites [docs/manifest.csv](docs/manifest.csv);
`verify_public_artifacts.py` re-checks the shipped files against it.

## Results map

Tables are referenced by LaTeX label, because the manuscript numbering is not final.
